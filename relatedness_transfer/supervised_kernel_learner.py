...
def parse_arguments():
    ...
    ap.add_argument("--kernel_agg", type=str, choices=["mean", "max", "wmean", "pwmean", "learned"], default="mean",
    ...
    # supervised kernel aggregation settings
    ap.add_argument("--sk_model", type=str, choices=["elasticnet", "xgb", "xgb_rank"],
                    default="elasticnet",
                    help="Regressor for learned kernel: Elastic Net (nonneg) or XGBoost (monotone).")
    ap.add_argument("--sk_alpha", type=float, default=1e-2,
                    help="ElasticNet alpha (overall regularization strength).")
    ap.add_argument("--sk_l1_ratio", type=float, default=0.2,
                    help="ElasticNet l1_ratio in [0,1].")
    ap.add_argument("--sk_use_fisher_z", action="store_true",
                    help="Train on Fisher-z( corr ) and invert with tanh at inference.")
    ap.add_argument("--xgb_depth", type=int, default=5,
                    help="XGBoost max_depth (if --sk_model xgb).")
    ap.add_argument("--xgb_estimators", type=int, default=600,
                    help="# trees (n_estimators) for XGBoost.")
    ap.add_argument("--xgb_lr", type=float, default=0.1,
                    help="XGBoost learning_rate.")
    ap.add_argument("--rank_ndcg_k", type=int, default=20,
                    help="NDCG@k used by the ranking objective (approximate top-k focus).")
    ap.add_argument("--rank_gain", type=str, choices=["relu", "shift01"], default="relu",
                    help="Ensure labels (gains) for ranking are nonnegative: relu=max(0,y) or shift01=(y-min)/(max-min).")
    ap.add_argument("--sk_rank_calibrate", action="store_true",
                    help="After training the ranker, apply 1-D isotonic calibration to map scores back to target similarities.")
...
def _fisher_z(s: np.ndarray) -> np.ndarray:
...
def _project_to_psd(K: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    K = 0.5 * (K + K.T)
    w, V = np.linalg.eigh(K)
    w = np.maximum(w, eps)
    Kpsd = (V * w) @ V.T
    Kpsd = 0.5 * (Kpsd + Kpsd.T)
    np.fill_diagonal(Kpsd, 1.0)
    return Kpsd.astype(np.float32)

def _build_target_O_sim(
    adata_train_target: ad.AnnData,
    target_label: str,
    control_label: str,
    metric: str = "corr",
) -> tuple[np.ndarray, list[str]]:
    O = [p for p in adata_train_target.obs[target_label].astype(str).unique() if p != control_label]
    del_O, _ = compute_deltas(adata_train_target, target_label, control_label)
    O = [p for p in O if p in del_O]
    if not O:
        return np.zeros((0, 0), dtype=np.float32), []
    Y = np.stack([np.asarray(del_O[p]).ravel() for p in O], 0).astype(np.float32)
    if metric == "corr":
        S = np.corrcoef(Y)
    elif metric == "cosine":
        S = 1.0 - pairwise_distances(Y, metric="cosine")
    else:
...
def _align_kernels_to_union(
    kernels_and_perts: list[tuple[np.ndarray, list[str]]],
) -> tuple[np.ndarray, list[str]]:
    """
    Returns S: (M,P,P) float32 with NaNs where pairs missing, and perts_union list.
    """
    # union order preserving
    union = []
    seen = set()
    for _, perts in kernels_and_perts:
        for p in perts:
            if p not in seen:
                seen.add(p)
                union.append(p)
    ...
    S = np.full((M, P, P), np.nan, dtype=np.float32)
    pos = {p: i for i, p in enumerate(union)}
    for m, (K, perts) in enumerate(kernels_and_perts):
        idx = {p: j for j, p in enumerate(perts)}
        keep = [pos[p] for p in perts if p in pos]
        if keep:
            keep_arr = np.asarray(keep, dtype=int)
            J = np.asarray([idx[union[i]] for i in keep_arr], dtype=int)
            K_sub = K[np.ix_(J, J)].astype(np.float32)
            # write directly into the (P,P) slice using broadcasted integer indices
            S[m, keep_arr[:, None], keep_arr] = K_sub
    return S, union

# ---------- O×O pair table ----------

def _build_pair_table_OO(
    S_stack: np.ndarray,       # (M,P,P) aligned, NaNs where missing
    perts_union: list[str],
    perts_O: list[str],
    S_tgt_OO: np.ndarray,      # (|O|,|O|)
    perts_O_order: list[str],
    use_fisher_z: bool = False,
) -> tuple[np.ndarray, np.ndarray, list[tuple[int,int]], list[str]]:
    """
    Build supervised dataset on O×O upper triangle.
    Returns X (N, M), y (N,), pairs [(i,j)], and feature_names (dataset_i).
    """
    M, P, _ = S_stack.shape
    pos_union = {p: k for k, p in enumerate(perts_union)}
    pos_O = {p: i for i, p in enumerate(perts_O_order)}
    # map O perts that exist in union
    O_in_union = [p for p in perts_O if p in pos_union and p in pos_O]
    if len(O_in_union) < 2:
        return np.zeros((0, M), np.float32), np.zeros((0,), np.float32), [], [f"ds{m}" for m in range(M)]

    # collect upper triangle pairs
    pairs = []
    feats = []
    ys = []
    for a_idx in range(len(O_in_union)):
        for b_idx in range(a_idx + 1, len(O_in_union)):
            pa, pb = O_in_union[a_idx], O_in_union[b_idx]
            ia, ib = pos_union[pa], pos_union[pb]
            ja, jb = pos_O[pa], pos_O[pb]
            # feature vector across datasets
            x = S_stack[:, ia, ib]  # shape (M,)
            if np.all(np.isnan(x)):
                continue  # no coverage at all
            feats.append(np.nan_to_num(x, nan=np.nan))  # keep NaNs; we'll impute later
            y = S_tgt_OO[ja, jb]
            ys.append(y)
            pairs.append((ia, ib))
    X = np.vstack(feats).astype(np.float32) if feats else np.zeros((0, M), np.float32)
    y = np.asarray(ys, dtype=np.float32)
    if use_fisher_z and y.size > 0:
        y = _fisher_z(y)
    feature_names = [f"ds{m}" for m in range(M)]
    return X, y, pairs, feature_names

# ---------- fit/predict: Elastic Net (nonneg) ----------

def _fit_elasticnet_nonneg(
    X: np.ndarray, y: np.ndarray, alpha: float, l1_ratio: float
):
    # Impute per-feature means; keep a missing-indicator is optional (skips for minimalism)
    model = Pipeline([
        ("imp", SimpleImputer(strategy="mean")),
        ("scaler", StandardScaler(with_mean=True, with_std=True)),
        ("enet", ElasticNet(alpha=float(alpha), l1_ratio=float(l1_ratio),
                            positive=True, fit_intercept=True, max_iter=5000, tol=1e-4))
    ])
    model.fit(X, y)
    return model

# ---------- fit/predict: XGBoost (monotone) ----------

def _fit_xgb_monotone(
    X: np.ndarray, y: np.ndarray, depth: int, n_estimators: int, lr: float
):
    # simple mean-impute for NaNs (xgb can also handle missing=np.nan if you pass it)
    X_imp = np.where(np.isfinite(X), X, np.nan)
    dtrain = xgb.DMatrix(X_imp, label=y, feature_names=[f"f{i}" for i in range(X.shape[1])])
    # monotone +1 for all source features
    ...
    params = dict(
        ...
        tree_method="hist",
        ndcg_exp_gain=False,  # <-- allow >31 relevance levels
    )
    bst = xgb.train(params, dtrain, num_boost_round=int(n_estimators))
    return ("xgb", bst)

def _predict_xgb(model_tuple, X: np.ndarray) -> np.ndarray:
    tag, bst = model_tuple
    X_imp = np.where(np.isfinite(X), X, np.nan)
    dmat = xgb.DMatrix(X_imp, feature_names=[f"f{i}" for i in range(X.shape[1])])
    return bst.predict(dmat)

# ---------- build full K* via learned f ----------

def build_learned_kernel(
    kernels_and_perts: list[tuple[np.ndarray, list[str]]],
    *,
    adata_train_target: ad.AnnData,
    target_label: str,
    ...
    sk_rank_calibrate: bool = False,
    rank_ndcg_k: int = 20,
) -> tuple[np.ndarray, list[str]]:
    """
    Learn f on O×O, then predict full K* over union P.
    """
    # 1) align all dataset kernels to union
    S_stack, perts_union = _align_kernels_to_union(kernels_and_perts)  # (M,P,P)
    M, P, _ = S_stack.shape

    # 2) target similarity on O×O
    S_tgt, perts_O_order = _build_target_O_sim(adata_train_target, target_label, control_label, metric="corr")
    perts_O = perts_O_order  # already filtered to those with deltas

    # 3) build pair table on O×O
    X, y, pairs, _ = _build_pair_table_OO(
        S_stack=S_stack, perts_union=perts_union, perts_O=perts_O,
        S_tgt_OO=S_tgt, perts_O_order=perts_O, use_fisher_z=use_fisher_z
    )
    if X.shape[0] == 0:
        # degenerate: fall back to simple mean
        K_mean = np.nanmean(S_stack, axis=0)
        K_mean = np.nan_to_num(K_mean, nan=0.0)
        K_mean = 0.5 * (K_mean + K_mean.T)
        np.fill_diagonal(K_mean, 1.0)
        return _project_to_psd(K_mean), perts_union

    # 4) fit model
    if regressor == "elasticnet":
        model = _fit_elasticnet_nonneg(X, y, alpha=alpha, l1_ratio=l1_ratio)
        def _predict_fn(Xin):  # closure
            return model.predict(Xin)
    elif regressor == "xgb":
        model = _fit_xgb_monotone(X, y, depth=xgb_depth, n_estimators=xgb_estimators, lr=xgb_lr)
        def _predict_fn(Xin):
            return _predict_xgb(model, Xin)
    elif regressor == "xgb_rank":
        # Build ranking dataset on O×O groups
        Xr, yr, grp, rank_pairs, _ = _build_rank_table_OO(
            S_stack=S_stack,
            perts_union=perts_union,
            S_tgt_OO=S_tgt,
            perts_O_order=perts_O,
            rank_gain="relu" if True else "shift01",  # or expose args.rank_gain if you prefer
            rank_levels=10,
        )
        if Xr.shape[0] == 0:
            ...
            K_mean = np.nanmean(S_stack, axis=0)
            K_mean = np.nan_to_num(K_mean, nan=0.0)
            K_mean = 0.5*(K_mean+K_mean.T); np.fill_diagonal(K_mean, 1.0)
            return _project_to_psd(K_mean), perts_union

        # NDCG@k must be <= group size; pick a safe k
        safe_k = int(max(1, min(rank_ndcg_k, max(1, len(perts_O) - 1))))
        model = _fit_xgb_rank_monotone(
            X=Xr, y=yr, group_sizes=grp,
            depth=xgb_depth, n_estimators=xgb_estimators, lr=xgb_lr,
            ndcg_k=safe_k
        )
        # Predict scores for ALL union pairs (upper triangle)
        rows, cols = np.triu_indices(P, k=1)
        X_all = S_stack[:, rows, cols].T.astype(np.float32)
        scores_all = _predict_xgb_rank(model, X_all)
        # Optional 1-D isotonic calibration to map scores -> target corr on O×O pairs used for training
        if sk_rank_calibrate:
            # Build target y_true vector for the exact O×O pairs used in ranking
            y_true_rank = []
            pos_O = {p: i for i, p in enumerate(perts_O)}
            for iu, ju in rank_pairs:
                p, q = perts_union[iu], perts_union[ju]
                y_true_rank.append(float(S_tgt[pos_O[p], pos_O[q]]) if (p in pos_O and q in pos_O) else 0.0)
            y_true_rank = np.asarray(y_true_rank, dtype=np.float32)
            # Also collect corresponding model scores for those pairs
            # Map from (min(i,j), max(i,j)) to position in scores_all
            idx_map = { (int(rows[k]), int(cols[k])): k for k in range(rows.shape[0]) }
            def _lookup_score(iu: int, ju: int) -> float:
                a, b = (iu, ju) if iu < ju else (ju, iu)
                return float(scores_all[idx_map[(a, b)]])
            scor_rank = np.asarray([_lookup_score(int(iu), int(ju)) for (iu, ju) in rank_pairs], dtype=np.float32)
            s_cal, _ = _isotonic_fit_predict(scor_rank, y_true_rank, scores_all)
            s_pred = np.clip(s_cal, -1.0, 1.0).astype(np.float32)
        else:
            # No calibration: rescale scores to [-1,1] by simple rank-based normalization
            # (keeps order for ranking; gives a usable similarity scale)
            r = scores_all
            r_min, r_max = float(np.nanmin(r)), float(np.nanmax(r))
            if np.isfinite(r_min) and np.isfinite(r_max) and r_max > r_min:
                s_pred = (2.0 * (r - r_min) / (r_max - r_min) - 1.0).astype(np.float32)
            else:
                s_pred = np.zeros_like(r, dtype=np.float32)
        # Assemble K*
        K_star = np.eye(P, dtype=np.float32)
        K_star[rows, cols] = s_pred
        K_star[cols, rows] = s_pred
        K_star = np.nan_to_num(K_star, nan=0.0)
        K_star = 0.5*(K_star+K_star.T); np.fill_diagonal(K_star, 1.0)
        K_star = _project_to_psd(K_star, eps=1e-6)
        return K_star, perts_union
    else:
        raise ValueError("regressor must be 'elasticnet', 'xgb', or 'xgb_rank'")

    # 5) predict ALL pairs over union (upper triangle), then symmetrize
    # build (N_pairs, M) feature matrix from S_stack across all p<q
    rows, cols = np.triu_indices(P, k=1)
    X_all = S_stack[:, rows, cols].T  # (N_pairs, M)
    X_all = np.asarray(X_all, dtype=np.float32)
    z_pred = _predict_fn(np.where(np.isfinite(X_all), X_all, np.nan if regressor=="xgb" else np.nan_to_num(X_all, nan=np.nan)))
    s_pred = _inv_fisher_z(z_pred) if use_fisher_z else z_pred
    # assemble K*
    K_star = np.eye(P, dtype=np.float32)
    K_star[rows, cols] = s_pred.astype(np.float32)
    K_star[cols, rows] = s_pred.astype(np.float32)
    # clean up and PSD project
    K_star = np.nan_to_num(K_star, nan=0.0)
    K_star = 0.5 * (K_star + K_star.T)
    np.fill_diagonal(K_star, 1.0)
    K_star = _project_to_psd(K_star, eps=1e-6)
    return K_star, perts_union

# ---------- Build grouped O×O data for ranking ----------

def _build_rank_table_OO(
    S_stack: np.ndarray,        # (M,P,P) source sims aligned to union, NaNs where missing
    perts_union: list[str],
    S_tgt_OO: np.ndarray,       # (|O|,|O|) target corr on O×O
    perts_O_order: list[str],   # order used in S_tgt_OO
    rank_gain: str = "relu",
    rank_levels: int = 10,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[tuple[int,int]], list[int]]:
    """
    Create a ranking dataset:
      - One query group per p in O: items are pairs (p,q) for q in O{p}
      - Features x_{pq} = [K_i(p,q)]_i (NaNs kept; XGBoost can accept them)
      - Labels y_{pq} transformed to nonnegative 'gains' for rank:ndcg
    Returns:
      X  : (N_items, M) float32
      y  : (N_items,) float32  (nonnegative gains)
      grp: (G,) int32 group_sizes (contiguous groups in XGB DMatrix)
      pairs: list of (iu, ju) indices into perts_union for each row in X
      mask_valid_groups: indices of O perts that yielded a non-empty group (for debugging)
    """
    M, P, _ = S_stack.shape
    pos_union = {p: k for k, p in enumerate(perts_union)}
    pos_O = {p: i for i, p in enumerate(perts_O_order)}
    ...
    for p in perts_O_order:
        if p not in pos_union:
            continue
        iu = pos_union[p]
        # collect all q in O except p that also exist in union
        q_list = [q for q in perts_O_order if q != p and q in pos_union]
        if not q_list:
            continue
        # features and labels for this group's items
        X_g = []
        y_g = []
        for q in q_list:
            ju = pos_union[q]
            x = S_stack[:, iu, ju]        # shape (M,)
            if np.all(np.isnan(x)):
                continue  # no coverage at all for this item
            X_g.append(x.astype(np.float32))
            y_g.append(float(S_tgt_OO[pos_O[p], pos_O[q]]))
            pairs.append((iu, ju))
        if not X_g:
            continue
        X_g = np.vstack(X_g).astype(np.float32)
        y_g = np.asarray(y_g, dtype=np.float32)
        # convert labels to nonnegative 'gains' for NDCG
        if rank_gain == "relu":
            y_g = np.maximum(0.0, y_g)
        else:  # "shift01": min-max over the group (falls back to relu if degenerate)
            lo, hi = float(np.min(y_g)), float(np.max(y_g))
            if hi > lo:
                y_g = (y_g - lo) / (hi - lo)
            else:
                y_g = np.maximum(0.0, y_g)
        # --- NEW: convert to integer relevance for NDCG ---
        # Step 1: make nonnegative (already done above via relu/shift01)
        # Step 2: map to integer gains (0..L-1) by descending rank within the group
        L = max(2, int(rank_levels))
        if y_g.size == 1:
            gains = np.array([0], dtype=np.int32)
        else:
            # ranks: 0 .. n-1, where n-1 is the largest y_g
            order = np.argsort(y_g)                  # ascending
            ranks = np.empty_like(order, dtype=np.int32)
            ranks[order] = np.arange(y_g.size, dtype=np.int32)
            # optional binning into L levels (preserves order, reduces noise)
            gains = np.floor((ranks / max(1, y_g.size - 1)) * (L - 1)).astype(np.int32)

        X_rows.append(X_g)
        y_rows.append(gains.astype(np.float32))  # XGBoost takes float, but they’re integers
        group_sizes.append(X_g.shape[0])
        valid_groups.append(pos_O[p])

    if not X_rows:
        return (np.zeros((0, M), np.float32),
                np.zeros((0,), np.float32),
                np.zeros((0,), np.int32),
                [], [])
    X = np.vstack(X_rows).astype(np.float32)
    y = np.concatenate(y_rows).astype(np.float32)
    grp = np.asarray(group_sizes, dtype=np.int32)
    return X, y, grp, pairs, valid_groups


# ---------- Fit XGBoost ranker with monotone constraints ----------

def _fit_xgb_rank_monotone(
    X: np.ndarray, y: np.ndarray, group_sizes: np.ndarray,
    depth: int, n_estimators: int, lr: float, ndcg_k: int
):
    # XGB can natively handle missing=np.nan; pass through NaNs as-is.
    dtrain = xgb.DMatrix(X, label=y, missing=np.nan, feature_names=[f"f{i}" for i in range(X.shape[1])])
    dtrain.set_group(group_sizes.tolist())

    constraints = "(" + ",".join(["1"] * X.shape[1]) + ")"  # +1 monotone per source feature
    params = dict(
        objective="rank:ndcg",
        eval_metric=f"ndcg@{int(ndcg_k)}",
        max_depth=int(depth),
        eta=float(lr),
        nthread=4,
        subsample=0.8,
...
def _predict_xgb_rank(model_tuple, X: np.ndarray) -> np.ndarray:
    tag, bst = model_tuple
    dmat = xgb.DMatrix(X, missing=np.nan, feature_names=[f"f{i}" for i in range(X.shape[1])])
    return bst.predict(dmat)


# ---------- 1-D isotonic calibration from scores -> target corr ----------

def _isotonic_fit_predict(
    scores_train: np.ndarray, y_true_train: np.ndarray, scores_all: np.ndarray
) -> tuple[np.ndarray, "object"]:
    """
    Fit a monotone 1-D calibration mapping model scores to target similarities.
    Returns calibrated_all, fitted_model.
    """
    # Guard against NaNs / inf
    mask = np.isfinite(scores_train) & np.isfinite(y_true_train)
    if mask.sum() < 10:
        # Not enough points to calibrate; return raw scores
        return scores_all, None
    iso = IsotonicRegression(y_min=-1.0, y_max=1.0, increasing=True, out_of_bounds="clip")
    iso.fit(scores_train[mask], y_true_train[mask])
    cal = iso.predict(scores_all)
    return cal, iso
...
def main():
    ...
    if args.kernel_agg == "learned":
        K_full, perts_union = build_learned_kernel(
            kernels_and_perts=kernels_and_perts,
            adata_train_target=adata_train_int,
            target_label=args.target_label,
            control_label=args.control_label,
            regressor=args.sk_model,
            alpha=args.sk_alpha,
            l1_ratio=args.sk_l1_ratio,
            xgb_depth=args.xgb_depth,
            xgb_estimators=args.xgb_estimators,
            xgb_lr=args.xgb_lr,
            use_fisher_z=args.sk_use_fisher_z,
            sk_rank_calibrate=args.sk_rank_calibrate,
            rank_ndcg_k=args.rank_ndcg_k,
        )
        print(f"[learned] Built supervised kernel K*: {K_full.shape} over {len(perts_union)} perts.")
    elif args.kernel_agg == "wmean":
