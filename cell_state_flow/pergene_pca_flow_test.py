...
def main(args):
    ...
    control_adata = adata # adata[adata.obs[args.target_label] == args.control_label].copy()
    ...
    if args.no_pca:
        ...
        # Keep these to whiten/unwhiten any group later
        mu_all, std_all = mu.copy(), std.copy()
    # Fallbacks for the no-PCA path so later code won’t break (identity “whitening”)
    if args.no_pca:
        mu_all = np.zeros((1, args.n_latent), dtype=train_latent.dtype)
        std_all = np.ones((1, args.n_latent), dtype=train_latent.dtype)
    ...
    # ------------------------------------------------------------------
    # NEW: Generate per-perturbation samples with same counts as original
    # ------------------------------------------------------------------
    print("\n--- Phase 3: Per-perturbation generation (one flow per perturbation) ---")
    pert_col = args.target_label
    if pert_col not in adata.obs.columns:
        raise ValueError(f"Column '{pert_col}' not found in adata.obs")
    pert_values = adata.obs[pert_col].astype('category')
    adata.obs[pert_col] = pert_values
    groups = pert_values.cat.categories.tolist()
    print(f"Found {len(groups)} perturbation groups: {groups[:8]}{' ...' if len(groups)>8 else ''}")

    # Prepare storage
    X_gen_all = []
    obs_rows  = []

    # We'll reuse your FlowModel; trained quickly per-perturbation on whitened PCA latents
    def fm_velocity_batch(flow_model, z1_batch, device):
        B, d = z1_batch.shape
        eps = torch.randn(B, d, device=device)
        t = torch.rand(B, 1, device=device)
        zt = (1 - t) * eps + t * z1_batch
        v_star = z1_batch - eps
        v_hat  = flow_model(zt, t.squeeze(1))
        return torch.mean((v_hat - v_star)**2)

    for g in groups:
        mask = (adata.obs[pert_col].values == g)
        Xg = full_data_matrix[mask]
        Ng = Xg.shape[0]
        if Ng == 0:
            continue
        print(f"[{g}] training flow on {Ng} cells...")
        # Project this group's data to PCA → whitened latent
        Zg = pca.transform(Xg)
        Zgw = (Zg - mu_all) / std_all

        # Train a small flow for this perturbation (velocity FM only; fast)
        flow_g = flow_g = FlowModel(n_latent=args.n_latent, n_hidden=args.n_hidden).to(DEVICE)
        opt_g  = torch.optim.Adam(flow_g.parameters(), lr=args.lr)
        loader_g = DataLoader(TensorDataset(torch.from_numpy(Zgw).float()), batch_size=args.batch_size, shuffle=True, drop_last=False)
        flow_g.train()
        for ep in range(max(1, args.epochs)):   # you can set a smaller --epochs for per-pert runs
            total = 0.0
            for (z1b,) in loader_g:
                z1b = z1b.to(DEVICE)
                loss = fm_velocity_batch(flow_g, z1b, DEVICE)
                opt_g.zero_grad()
                loss.backward()
                opt_g.step()
                total += loss.item() * z1b.size(0)
            if (ep+1) % 10 == 0 or ep == 0:
                print(f"  ep {ep+1:03d}  FM loss: {total/Ng:.6f}")
        flow_g.eval()

        # Generate exactly Ng samples in whitened latent
        with torch.no_grad():
            z0   = torch.randn(Ng, args.n_latent, device=DEVICE)
            def ode_func(t, z):
                return flow_g(z, t.expand(z.size(0)))
            tspan = torch.tensor([0.0, 1.0], device=DEVICE)
            Zgw_gen = odeint(ode_func, z0, tspan, method='dopri5')[1].cpu().numpy()   # (Ng, d)

        # Optional: clamp whitened latent radius to training support (p99.5)
        norms = np.linalg.norm(Zgw, axis=1)
        safe_r = float(np.percentile(norms, 99.5)) if Ng > 50 else float(np.percentile(norms, 100.0))
        gen_norms = np.linalg.norm(Zgw_gen, axis=1, keepdims=True) + 1e-12  # (Ng, 1)
        # Build a per-row scale factor; ≤1 when outside the safe radius
        scale = np.ones_like(gen_norms, dtype=Zgw_gen.dtype)
        too_big = gen_norms > safe_r
        scale[too_big] = (safe_r / gen_norms[too_big]).astype(Zgw_gen.dtype)
        Zgw_gen *= scale  # broadcast (Ng,1) × (Ng,d) → (Ng,d)

        # Unwhiten → inverse PCA → clamp to ≥ 0 (log1p domain)
        Zg_gen = Zgw_gen * std_all + mu_all
        Xg_gen = pca.inverse_transform(Zg_gen)
        Xg_gen = np.maximum(Xg_gen, 0.0)

        # Append to global buffers with obs
        X_gen_all.append(Xg_gen)
        obs_rows.extend([{'source': 'Generated', pert_col: g}] * Ng)

    # Stack generated across all perturbations
    ...
    # Prepare ORIGINAL slice in same order as adata, tag with 'Original'
    X_orig = full_data_matrix
    if hasattr(X_orig, "toarray"):
        X_orig = X_orig.toarray()
    X_orig = np.asarray(X_orig)
    X_orig = np.maximum(X_orig, 0.0)
    obs_orig = adata.obs.copy()
    obs_orig = obs_orig[[pert_col]].copy()
    obs_orig['source'] = pd.Categorical(['Original'] * adata.n_obs)

    # Concatenate ORIGINAL + GENERATED into one AnnData
    X_all = np.vstack([X_orig, X_gen_all])
    obs_all = pd.concat([
        obs_orig.reset_index(drop=True),
        obs_gen.reset_index(drop=True)
    ], axis=0, ignore_index=True)
    # var carried over from input
    ...
    if args.output_h5ad:
        ...
        print(f"Wrote per-perturbation ORIGINAL+GENERATED to '{args.output_h5ad}'")
