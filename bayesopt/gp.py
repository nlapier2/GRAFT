...
def parse_arguments():
    ap = argparse.ArgumentParser(description="Multi dataset GP for relatedness transfer.")
    ...
    ap.add_argument("--lowrank_pca_r", type=int, default=0,
                    help="If >0, run GP in a rank-r PCA space learned from target TRAIN deltas, then reconstruct.")
    ...
    ap.add_argument("--impute_missing_from_first", action="store_true", help="GP-impute missing perturbation rows using first external.")
...
def create_kernel(
    ...
    # --- Optional: GP-impute missing perts from the first external (donor) ---
...
def gp_predict_from_external(
    ...
    if K_full is None or perts_all is None:
        raise ValueError("gp_predict_from_external requires a precomputed kernel: pass K_full and perts_all.")
    ...
    if K_full is None or perts_all is None:
        raise ValueError("gp_predict_from_external now requires a precomputed kernel: pass K_full and perts_all.")
    K = K_full.astype(np.float32, copy=False)

    KOO = K[np.ix_(iO, iO)]
    KUO = K[np.ix_(iU, iO)]

    # ---- GP solve & prediction (full rank or low-rank PCA) ----
...
def main():
    ...
    # These are used for the GP call so indices/names match the kernel exactly.
    ...
    # Run GP ONLY on the intersected views; get predictions for intersected perts
    pred_gp_ev, true_gp_ev, names_gp_ev, ctrl_mean_gp, pred_delta_var_ev = gp_predict_from_external(
    ...
    for j, p in enumerate(names_gp_ev):
        if p in name2row_ev:
            pred_ev[name2row_ev[p], :] = pred_gp_ev[j, :]
    # Optionally DROP OOV perts (not present in intersection)
    if not args.keep_oov_perts:
        keep = np.array([p in set(names_gp_ev) for p in names_ev])
        pred_ev, true_ev = pred_ev[keep], true_ev[keep]
        names_ev = [p for (p, k) in zip(names_ev, keep) if k]
    pred_ev_orig = pred_ev.copy() # Store original GP predictions
    ...
    if args.eval_on_train and (adata_test is not None):
        ...
        pred_gp_tr, _true_gp_tr, names_gp_tr, _ctrl_ignored, _pred_delta_var_tr = gp_predict_from_external(
            adata_train=adata_train_core,
        ...
        for j, p in enumerate(names_gp_tr):
            if p in name2row_tr:
                pred_tr[name2row_tr[p], :] = pred_gp_tr[j, :]
        if not args.keep_oov_perts:
            keep = np.array([p in set(names_gp_tr) for p in names_tr])
